...
public interface UrgenciaRepository extends JpaRepository<Urgencia, Long> {
    @Query("SELECT u FROM Urgencia u LEFT JOIN u.doctor d WHERE u.atendida = :atendida")
    ListaSimple<Urgencia> getUrgenciaAndDoctor(@Param("atendida") boolean atendida);
}
